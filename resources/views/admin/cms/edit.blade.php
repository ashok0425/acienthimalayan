	...
	<div class="row">
		<div class="col-md-6">
			<div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
				<label >Title</label>
				<input type="text" name="title" id="" class="form-control" value="{{ $menu->title }}" placeholder="Enter CMS page title">
				@if($errors->has('title'))
		...
		<div class="form-group{{ $errors->has('parent_id') ? ' has-error' : '' }}">
			...
			<div class="d-flex">
				<label >
					<input type="radio" name="main_or_sub" class="main_or_sub" value="1"  @if ($menu->main_or_sub==1)
                        checked
                    @endif> Main
				</label>
				<label class="mx-3">
					<input type="radio" name="main_or_sub" class="main_or_sub" value="0" @if ($menu->main_or_sub==0)
                        checked
                    @endif> Sub
				</label>
			</div>
		...
		<div class="col-md-12">
			...
			<div  id="maincms" class="  @if ($menu->main_or_sub==0)
                d-block
                @else 
                d-none
            @endif">	
		...
		<div class="col-md-6">
				...
				<div class="form-group{{ $errors->has('position') ? ' has-error' : '' }}">
					...
					<label > Position:</label>
						<input type="number" name="position" id="" placeholder="Enter Postion value" required class="form-control" value="{{ $menu->position }}"> 
            ...
            <div class="form-group">    
                <label class="ckbox ckbox-success">
                   <p>Hide in Header</p>
					...
					<input type="checkbox" name="hide_header" id="" value="1"  class="form-control" @if ($menu->hide_header==1)
                        checked
                    @endif> 
                </label>
